...
const navItems = ["Visão geral", "Atualizar custos", "Minhas bases", "Minhas receitas", "Vendas", "Despesas", "Ajustes"];
...
export default function Home() {
  ...
  const renderCosts = () => (
    <div className="module-stack"><section className="module-header"><div><span className="section-label">ATUALIZAÇÃO RÁPIDA</span><h2>O fornecedor mudou o preço? Atualize uma vez.</h2><p>Selecione o ingrediente, informe a nova compra e todas as bases e receitas ligadas a ele serão recalculadas.</p></div></section>
      ...
      <section className="pricing-grid"><form className="panel form-panel" onSubmit={addIngredient}><div><span className="step-number">NOVO CADASTRO</span><h3>Adicionar ingrediente</h3></div><label>Nome<input value={ingredientForm.name} onChange={(e) => setIngredientForm({ ...ingredientForm, name: e.target.value })} placeholder="Ex.: Leite em pó" /></label><div className="form-row three"><label>Quantidade<input type="number" min="0.01" step="0.01" value={ingredientForm.purchaseQty} onChange={(e) => setIngredientForm({ ...ingredientForm, purchaseQty: Number(e.target.value) })} /></label><label>Unidade<select value={ingredientForm.unit} onChange={(e) => setIngredientForm({ ...ingredientForm, unit: e.target.value as Unit })}><option value="g">gramas</option><option value="ml">mililitros</option><option value="un">unidades</option></select></label><label>Valor pago<input type="number" min="0.01" step="0.01" value={ingredientForm.purchaseCost || ""} onChange={(e) => setIngredientForm({ ...ingredientForm, purchaseCost: Number(e.target.value) })} /></label></div><label>Perda estimada (%)<input type="number" min="0" max="80" step="0.1" value={ingredientForm.wastePct} onChange={(e) => setIngredientForm({ ...ingredientForm, wastePct: Number(e.target.value) })} /></label><button className="secondary-button full" type="submit">Cadastrar ingrediente</button></form>
        <article className="panel ingredient-list"><div className="panel-heading"><div><span className="step-number">MEUS INGREDIENTES</span><h3>Valores atuais</h3></div><span className="soft-pill">{data.ingredients.length} itens</span></div><div className="table-scroll">{data.ingredients.map((item) => <div className="data-row ingredient-data-row" key={item.id}><div><strong>{item.name}</strong><small>{item.purchaseQty} {item.unit} por {money.format(item.purchaseCost)} · {money.format(ingredientCost(item))}/{item.unit}</small></div><div className="row-actions"><button className="update-button" type="button" onClick={() => { selectIngredientForUpdate(item.id); window.scrollTo({ top: 0, behavior: "smooth" }); }}>Atualizar</button><button className="delete-button" type="button" onClick={() => remove("ingredients", item.id)}>Excluir</button></div></div>)}</div></article></section>
    </div>
  );
  ...
  return (
    <main className="app-shell"><aside className="sidebar"><div className="brand"><span className="brand-mark">DL</span><div><strong>Doce Lucro</strong><small>Gestão para confeitaria</small></div></div><nav aria-label="Navegação principal">{navItems.map((item) => <button className={active === item ? "nav-item active" : "nav-item"} key={item} onClick={() => setActive(item)} type="button"><span className="nav-dot" />{item}</button>)}</nav><div className="sidebar-note"><span>Meta do mês</span><strong>{stats.progress.toFixed(0)}% alcançada</strong><div className="mini-progress"><span style={{ width: `${stats.progress}%` }} /></div><small>{stats.missing ? `Faltam ${money.format(stats.missing)}` : "Meta concluída!"}</small></div></aside>
      <section className="workspace"><header className="topbar"><div><p className="eyebrow">{dateHeading}</p><h1>{active === "Visão geral" ? `Bom trabalho, ${data.settings.businessName}!` : active}</h1><p>{active === "Visão geral" ? "Acompanhe o que entrou, o que saiu e quanto ficou para você." : "Alterações são salvas automaticamente neste aparelho."}</p></div><div className="topbar-actions"><button className="settings-shortcut" type="button" onClick={() => setActive("Ajustes")}>Ajustes</button><button className="primary-button" type="button" onClick={() => setActive("Atualizar custos")}>Atualizar custo</button></div></header>{active === "Visão geral" && renderDashboard()}{active === "Atualizar custos" && renderCosts()}{active === "Minhas bases" && renderBases()}{active === "Minhas receitas" && renderRecipes()}{active === "Vendas" && renderSales()}{active === "Despesas" && renderExpenses()}{active === "Ajustes" && renderSettings()}</section>
      <nav className="mobile-nav five-items" aria-label="Navegação móvel">{["Visão geral", "Atualizar custos", "Minhas bases", "Minhas receitas", "Vendas"].map((item) => <button className={active === item ? "active" : ""} key={item} onClick={() => setActive(item)} type="button"><span>{item.slice(0, 1)}</span>{item === "Visão geral" ? "Início" : item.replace("Minhas ", "")}</button>)}</nav>{notice && <div className="toast" role="status">{notice}</div>}</main>
  );
}
