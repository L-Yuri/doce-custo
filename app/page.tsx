...
export default function Home() {
  ...
  function resetProductForm() {
    ...
    setProductForm({ name: "", yield: 0, packagingIngredientId: packagingIngredients[0]?.id ?? "", laborHours: 1, otherBatchCost: 0, desiredMargin: 30, feePct: 4, reservePct: 3, recipe: [{ kind: data.bases.length ? "base" : "ingredient", itemId: data.bases[0]?.id ?? data.ingredients[0]?.id ?? "", quantity: 100 }] });
  }
  ...
  const renderBases = () => (
    <div className="module-stack">
      ...
      <div className="production-grid">
        <form className="panel product-form" onSubmit={saveBase}>
        ...
        <section className="base-list">{data.bases.length ? data.bases.map((base) => { const cost = baseCost(base, data.ingredients, data.settings.hourlyRate); return <article className="panel recipe-detail-card" key={base.id}><div className="panel-heading"><div><span className="type-pill base-type">BASE</span><h3>{base.name}</h3><p>Rendimento obtido: {base.yieldQty} {base.unit}</p></div><div className="recipe-price"><small>Custo da base</small><strong>{money.format(cost.batchTotal)}</strong><span>{money.format(cost.unitCost)}/{base.unit}</span></div></div><div className="composition-list"><span>Composição</span>{base.recipe.map((line) => { const ingredient = data.ingredients.find((item) => item.id === line.ingredientId); return <div key={`${base.id}-${line.ingredientId}`}><span>{ingredient?.name ?? "Ingrediente removido"}</span><strong>{line.quantity} {ingredient?.unit ?? ""}</strong></div>; })}<div><span>Mão de obra</span><strong>{money.format(cost.labor)}</strong></div></div><div className="recipe-card-actions"><button className="secondary-button" type="button" onClick={() => editBase(base)}>Editar base</button><button className="delete-button" type="button" onClick={() => remove("bases", base.id)}>Excluir base</button></div></article>; }) : <article className="panel empty-state"><h3>Nenhuma base cadastrada</h3><p>Crie seu primeiro recheio ou preparo usando o formulário.</p></article>}</section>
      </div>
    </div>
  );
