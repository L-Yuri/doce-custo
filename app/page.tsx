...
export default function Home() {
  ...
  const renderSettings = () => (
    <div className="module-stack">
      <section className="module-header"><div><span className="section-label">AJUSTES</span><h2>Personalize o Doce Lucro</h2></div></section>
      <section className="panel settings-panel">
        <div className="form-row two"><label>Nome do negócio<input value={data.settings.businessName} onChange={(e) => setData({ ...data, settings: { ...data.settings, businessName: e.target.value } })} /></label><label>Meta mensal<input type="number" min="0" value={data.settings.monthlyGoal} onChange={(e) => setData({ ...data, settings: { ...data.settings, monthlyGoal: Number(e.target.value) } })} /></label></div>
        <div className="form-row two"><label>Dias de trabalho no mês<input type="number" min="1" max="31" value={data.settings.workDays} onChange={(e) => setData({ ...data, settings: { ...data.settings, workDays: Number(e.target.value) } })} /></label><label>Valor da sua hora<input type="number" min="0" value={data.settings.hourlyRate} onChange={(e) => setData({ ...data, settings: { ...data.settings, hourlyRate: Number(e.target.value) } })} /></label></div>
        <div className="markup-setting"><div><span className="section-label">META DE PRECIFICAÇÃO</span><h3>Markup padrão desejado</h3><p>O preço sugerido de todas as receitas será o custo final multiplicado por esta meta.</p></div><label>Multiplicador<input type="number" min="1" max="20" step="0.1" value={data.settings.targetMarkup} onChange={(e) => setData({ ...data, settings: { ...data.settings, targetMarkup: Math.max(1, Number(e.target.value)) } })} /></label><div className="markup-example"><span>Exemplo com custo de R$ 10,00</span><strong>{money.format(10 * data.settings.targetMarkup)}</strong><small>R$ 10,00 × {data.settings.targetMarkup.toLocaleString("pt-BR")}x</small></div></div>
        <div className="backup-box"><div><strong>Backup e sincronização</strong><p>Exporte um arquivo quando quiser. Com a conta conectada, cada alteração também é enviada automaticamente para a nuvem.</p><small className={`backup-sync-status ${cloudStatus}`}>{cloudStatusLabel}{cloudEmail ? ` · ${cloudEmail}` : ""}</small></div><div className="backup-actions"><button className="secondary-button" type="button" onClick={exportData}>Exportar backup</button>{cloudStatus === "signed-out" ? <a className="secondary-button sync-settings-button" href={signInPath}>Entrar e sincronizar</a> : <button className="secondary-button sync-settings-button" type="button" disabled={cloudStatus === "checking" || cloudStatus === "syncing"} onClick={() => cloudStatus === "offline" ? window.location.reload() : setData((old) => ({ ...old }))}>{cloudStatus === "syncing" ? "Sincronizando..." : cloudStatus === "checking" ? "Verificando..." : cloudStatus === "offline" ? "Tentar sincronizar" : "Sincronizar agora"}</button>}<label className="file-button">Restaurar backup<input type="file" accept="application/json" onChange={importData} /></label></div></div>
        <button className="danger-button" type="button" onClick={() => { setData(initialData); localStorage.removeItem("doce-lucro-data-v1"); localStorage.removeItem("doce-lucro-data-v2"); localStorage.removeItem("doce-lucro-data-v3"); localStorage.removeItem("doce-lucro-data-v4"); localStorage.removeItem("doce-lucro-data-v5"); flash("Dados de exemplo restaurados sem movimentações financeiras."); }}>Restaurar dados de exemplo</button>
      </section>
    </div>
  );
